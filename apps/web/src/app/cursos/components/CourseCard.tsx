...
/* Types */
import type { Course } from '@voz-y-cuento/types';
/* Utils */
...
export default function CardCourse({ course }: CardCoursesProps) {
	return (
		<li className="flex w-[320px] flex-col rounded-2xl border border-solid border-gray-900 bg-white shadow-lg transition-transform ease-in-out will-change-transform hover:scale-[1.03] active:border-cyan-600 lg:w-[356px]">
			<NextLink href={`/cursos/${course.slug}`}>
				{/* Card Image */}
				<div className="relative h-[180px] w-full lg:h-[200px]">
					{course.cardImage && (
						<NextImage
							alt={course.cardImage.alternativeText ?? course.name}
							className="rounded-t-2xl object-cover"
							fill
							src={getImageUrl(course.cardImage.url)}
						/>
					)}
					...
					{course.category && (
						<aside className="absolute right-2 top-2 rounded-full bg-cyan-100 px-4 py-2 font-bold text-black">
							{course.category === 'Tematico' ? 'Temático' : course.category}
						</aside>
					)}
				...
				<div className="h-[156px] px-4 py-2 lg:h-[171px] lg:py-4">
					...
					{course.shortDescription && (
						<p className="mt-2 line-clamp-3 text-ellipsis">
							{course.shortDescription}
						</p>
					)}
				</div>
